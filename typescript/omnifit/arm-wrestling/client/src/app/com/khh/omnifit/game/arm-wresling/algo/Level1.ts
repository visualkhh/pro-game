...
export class Level1 extends Algo {
  ...
  onCreate(data?: any): Algo {
    this.concentrationSubscription = DeviceManager.getInstance().headsetConcentrationSubscribe((concentration) => {
      if (RandomUtil.random(0, 100) <= 30) {
        concentration = Math.min(10, Math.floor(RandomUtil.random(concentration, 10) + 1));
      } else {
        concentration = Math.max(0, Math.floor(RandomUtil.random(0, concentration)));
      }

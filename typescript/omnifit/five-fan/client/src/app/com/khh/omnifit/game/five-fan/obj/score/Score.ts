...
export class Score extends AWObj {
  ...
  onDraw(context: CanvasRenderingContext2D): void {
    ...
    const headsetConcentration = Math.min(10, this.room ? this.room.other.headsetConcentration : 0);
  ...
  onStart(data?: any) {
    ...
    //집중도
    this.roomDetailSubscription = this.stage.eventObservable(AWStageEvent.EVENT_ROOM_DETAIL).filter( (it) => !ValidUtil.isNullOrUndefined(it.other) && !ValidUtil.isNullOrUndefined(it.other)).subscribe( (room: Room) => {
      this.room = room;
    });
